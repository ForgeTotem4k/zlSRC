...
public interface Logger {
  ...
  default LoggingEventBuilder makeLoggingEventBuilder(Level paramLevel) {
    return (LoggingEventBuilder)new DefaultLoggingEventBuilder(this, paramLevel);
  }
  
  @CheckReturnValue
  default LoggingEventBuilder atLevel(Level paramLevel) {
    return isEnabledForLevel(paramLevel) ? makeLoggingEventBuilder(paramLevel) : NOPLoggingEventBuilder.singleton();
  }
  
  default boolean isEnabledForLevel(Level paramLevel) {
    int i = paramLevel.toInt();
    switch (i) {
      case 0:
    ...
    throw new IllegalArgumentException("Level [" + paramLevel + "] not recognized.");
  }
...
/* Location:              C:\Users\forge\zLauncher\Launcher.jar!\org\slf4j\Logger.class
